...
const Component = (props) => {
    ...
    const OnSubmit = async () => {
        let rslt, data, changes = [], petServiceId, petservice, numfields;

        petservice = row['petservice'];
        petServiceId = row['petservice'].find((x) => x.type === 'keyid').value;
                ...
                if (changes.length > 0) {
                    ...
                    if (tmp.length > 0) {
                        ...
                        if (rslt.status) {
                            ...
                            if (Helper.IsNullValue(mapItem.navpropname)) petServiceId = rslt.id;
        ...
        global.AlertPopup("success", "PetService is updated successfully!");
        setShowUpdate(false);
        NavigateTo(-1);
    }
